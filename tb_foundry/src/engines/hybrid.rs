...
pub fn run(
    ...
    info!("Starting Alpha Foundry (Dynamic Lazy Cache Engine)");
    ...
    // Cache tracker
    let mut known_indicators = HashSet::new();
    ...
    for gen_idx in 1..=generations {
        ...
        // STEP A: JIT Feature Appender
        // Scan population for new indicators and append them to the master DataFrame
        let mut new_indicator_nodes = Vec::new();
        for sketch in &population {
            sketch.extract_indicator_nodes(&mut known_indicators, &mut new_indicator_nodes);
        }
        
        if !new_indicator_nodes.is_empty() {
            let mut new_columns = Vec::with_capacity(new_indicator_nodes.len());
            for node in new_indicator_nodes {
                let col_name = node.to_string();
                // Compile the indicator purely from scratch using the tb_core compiler
                if let Ok(expr) = tb_core::ast_compiler::compile_ast_to_polars(&node) {
                    new_columns.push(expr.alias(&col_name));
                }
            }
            
            // Append all new indicators to the DataFrame in one highly parallel pass
            prepared_data = prepared_data.lazy()
                .with_columns(new_columns)
                .collect()
                .map_err(|e| FoundryError::Evaluation(format!("Failed to compute new cache columns: {}", e)))?;
        }
        
        // STEP B: Evaluate strategies using the Cache Compiler
        ...
        // Execute queries (Polars will instantly resolve these since the math is already cached!)
    ...
    info!("Alpha Foundry optimization complete. Cache Size: {} indicators", known_indicators.len());
